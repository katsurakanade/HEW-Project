...
// アクションゲージ最大値、最小値
static const float VALUE_MAX = 70.0;
static const float VALUE_MIN = -20.0;

// アクションゲージ状態値
static const float VALUE_OVER = VALUE_MAX * 0.9;
static const float VALUE_GREAT = VALUE_MAX * 0.65;
...
void ActionSlot::Load() {
	...
	Hand.LoadTexture(TextureDict["arm"]);
...
void ActionSlot::Update(double stamina,bool flag) {
	...
	if (!flag) {
		...
		// 常時減らす
		if (Value > VALUE_MIN && Value < VALUE_MAX + 5.0f) {
			Value -= VALUE_DOWNSEC;
		}
	...
	if (FireBase.Object.Rotate >= 0) {
		...
		FireBase.Object.Pos.y = Hand.Object.Pos.y + 30 + Fire_Offset.y + (Hand.Object.Rotate * 100) - ((Scale.y + 0.8) * Value);
	}
